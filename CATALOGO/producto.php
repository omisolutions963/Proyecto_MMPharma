...
<main class="max-w-[1600px] mx-auto px-12 pb-24 relative">
  ...
  <div class="grid md:grid-cols-2 gap-10 mb-16">
    ...
    <!-- Imagen del producto -->
    <div class="bg-surface rounded-[2rem] flex items-center justify-center min-h-[400px] p-12 relative" data-aos="fade-right">
      <?php if ($p['tipo'] === 'RED FRIA'): ?>
      <span class="absolute top-4 left-4 inline-flex items-center gap-1 px-3 py-1.5 bg-tertiary/20 text-tertiary-light text-xs font-bold rounded-full">
      ...
      <?php else: ?>
        <div class="text-center">
          <span class="material-symbols-outlined text-8xl text-slate-500">medication</span>
    ...
    <div class="flex flex-col justify-between" data-aos="fade-left">
      ...
      <div>
        ...
        <!-- Nombre -->
        <h1 class="text-2xl font-extrabold text-white tracking-tight mb-3 leading-tight">
          <?= htmlspecialchars($p['nombre']) ?>
        </h1>

        <!-- Sustancia activa -->
        <div class="flex items-start gap-2 mb-6">
          <span class="material-symbols-outlined text-primary-light text-lg mt-0.5">science</span>
          <div>
            <p class="text-xs font-bold uppercase tracking-wider text-slate-500">Sustancia activa</p>
            <p class="text-sm text-slate-300"><?= htmlspecialchars($p['sustancia'] ?? 'No especificada') ?></p>
          </div>
        </div>
        ...
        <?php if ($is_admin || $is_cliente): ?>
            ...
            <div class="bg-primary/20 p-5 rounded-2xl transition-transform hover:-translate-y-1">
                <p class="text-[10px] font-black text-primary-light uppercase tracking-widest mb-1 opacity-80">Farmacia</p>
                <p class="text-2xl font-black text-white">$<?= number_format($p['precio_farmacia'], 2) ?></p>
            </div>
            <div class="bg-secondary/20 p-5 rounded-2xl transition-transform hover:-translate-y-1">
                <p class="text-[10px] font-black text-secondary-light uppercase tracking-widest mb-1 opacity-80">Distribuidor</p>
                <p class="text-2xl font-black text-white">$<?= number_format($p['precio_distribuidor'], 2) ?></p>
            </div>
            <div class="bg-tertiary/20 p-5 rounded-2xl transition-transform hover:-translate-y-1">
                <p class="text-[10px] font-black text-tertiary-light uppercase tracking-widest mb-1 opacity-80">Empresa</p>
            ...
            <div class="mb-10 animate-reveal">
              ...
              <?php 
                $box_class = 'bg-primary/20 text-primary-light';
                if ($cliente_tipo === 'DISTRIBUIDORA') $box_class = 'bg-secondary/20 text-secondary-light';
                elseif ($cliente_tipo === 'EMPRESA') $box_class = 'bg-tertiary/20 text-tertiary-light';
              ?>
        ...
        <div class="space-y-4">
          <div class="flex items-end gap-4">
            <div class="flex flex-col">
              ...
              <div class="flex items-center bg-background rounded-xl h-[58px] px-2 w-32">
                <button type="button" onclick="const qty=document.getElementById('qty'); qty.value=Math.max(1, parseInt(qty.value)-1);" class="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-primary-light transition-colors hover:bg-white/5 rounded-lg">
                  <span class="material-symbols-outlined">remove</span>
                </button>
                <input type="number" id="qty" value="1" min="1" class="w-full text-center font-bold text-white bg-transparent border-none focus:ring-0 p-0 text-lg">
                <button type="button" onclick="const qty=document.getElementById('qty'); qty.value=parseInt(qty.value)+1;" class="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-primary-light transition-colors hover:bg-white/5 rounded-lg">
                  <span class="material-symbols-outlined">add</span>
                </button>
              </div>
            </div>
            
            <button type="button" onclick="anadirMultipleAlCarrito()" class="flex-1 h-[58px] bg-primary text-white font-bold rounded-xl hover:bg-primary-light hover:text-surface hover:-translate-y-1 active:scale-95 transition-all text-base flex items-center justify-center gap-2">
              <span class="material-symbols-outlined text-xl">add_shopping_cart</span>
              Añadir al Carrito
            </button>
          </div>
          <a href="catalogo.php" class="w-full h-[50px] bg-surface text-slate-300 font-bold rounded-xl hover:bg-white/5 transition-all text-sm flex items-center justify-center gap-2 ">
        ...
        <div class="space-y-4">
          <div class="bg-primary/5 rounded-xl p-5 flex items-center gap-4">
            <span class="material-symbols-outlined text-primary-light text-2xl">admin_panel_settings</span>
            <p class="text-sm text-slate-300 leading-relaxed">
              <strong class="text-primary-light block mb-1">Modo Administrador</strong>
        ...
        <div class="space-y-4">
          <div class="bg-blue-500/10 rounded-xl p-5 flex items-center gap-4">
            <span class="material-symbols-outlined text-primary-light text-2xl">lock</span>
            <p class="text-sm text-slate-300 leading-relaxed">
              <strong class="text-primary-light block mb-1">¿Quieres ver precios y cotizar este producto?</strong>
              Inicia sesión como cliente o solicita acceso para visualizar nuestro catálogo con precios.
            </p>
          </div>
          <div class="flex flex-col sm:flex-row gap-4">
            <a href="../LOGIN/login.php"
               class="flex-1 h-[58px] bg-primary text-white font-bold rounded-xl hover:bg-primary-light hover:text-surface hover:-translate-y-1 active:scale-95 transition-all text-base flex items-center justify-center">
              Iniciar sesión para acceder
            </a>
            <a href="catalogo.php"
               class="px-8 h-[58px] bg-surface text-slate-300 font-bold rounded-xl hover:bg-white/5 hover:-translate-y-1 transition-all text-base flex items-center justify-center gap-2">
              <span class="material-symbols-outlined text-lg">arrow_back</span>
              Volver
            </a>
          </div>
  ...
  <div>
    <h2 class="text-lg font-bold text-white mb-6">Productos con sustancia similar</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6" data-aos="fade-up">
      ...
      <a href="producto.php?id=<?= $r['id'] ?>"
         class="bg-surface rounded-[3rem] transition-all duration-300 p-8 flex flex-col group animate-fade-up min-h-[420px] hover:-translate-y-2">
        
        <!-- Contenedor de Imagen -->
        <div class="w-full aspect-square bg-background rounded-2xl flex items-center justify-center mb-6 relative group-hover:scale-105 transition-transform duration-500 overflow-hidden">
          <?php if (!empty($r['imagen']) && $r['imagen'] !== 'PENDIENTE'): ?>
            <img src="imagenes/productos/<?= htmlspecialchars($r['imagen']) ?>"
                 class="w-full h-full object-contain p-4">
          <?php else: ?>
            <span class="material-symbols-outlined text-slate-500 text-7xl">medication</span>
          <?php endif; ?>
          <?php if ($r['tipo'] === 'RED FRIA'): ?>
            <span class="absolute top-4 right-4 inline-flex items-center justify-center w-10 h-10 bg-tertiary/20 text-tertiary-light rounded-full">
        ...
        <!-- Info -->
        <div class="flex-1">
          <p class="text-sm font-black text-primary-light leading-tight mb-2 group-hover:text-white transition-colors uppercase tracking-tight">
        ...
        <div class="flex items-center justify-between">
          <p class="text-xl font-black text-primary-light">$<?= number_format($r[$precio_campo] ?? $r['precio_farmacia'], 2) ?></p>
          ...
          <button type="button" 
                  ...
                  class="w-12 h-12 rounded-2xl bg-white/5 text-primary-light hover:bg-primary-light hover:text-surface transition-all flex items-center justify-center">
            <span class="material-symbols-outlined text-2xl">shopping_cart</span>
          </button>
          <?php elseif ($is_admin): ?>
          <div class="w-12 h-12 rounded-2xl bg-background text-slate-500 flex items-center justify-center" title="Modo Administrador">
            <span class="material-symbols-outlined text-2xl">admin_panel_settings</span>
          </div>
          <?php else: ?>
          <div class="w-12 h-12 rounded-2xl bg-background text-slate-500 flex items-center justify-center">
  ...
  <div class="absolute inset-0 z-40 flex items-center justify-center bg-background/80 backdrop-blur-[4px]">
    <div class="max-w-md w-full mx-4 bg-surface p-10 rounded-[2.5rem] text-center animate-reveal">
      ...
      <h2 class="text-3xl font-black text-primary-light tracking-tight mb-4">Detalles exclusivos</h2>
      ...
      <a href="catalogo.php" class="inline-block mt-6 text-slate-500 font-bold text-sm hover:text-primary-light transition-colors">
        Volver al catálogo
      </a>
    </div>
  </div>
