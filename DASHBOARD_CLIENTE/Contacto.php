...
<main class="ml-64 mt-16 p-8 min-h-screen w-[calc(100%-16rem)]" style="background:#071628">
  ...
  <div class="relative">
  <select name="categoria" class="w-full bg-surface-container-low border border-outline-variant/50 text-white rounded-xl pl-4 pr-10 py-3 text-sm appearance-none focus:border-primary outline-none transition-all">
  ...
  <input type="text" name="cotizacion" placeholder="# COT-XXXX-XXX" class="w-full bg-surface-container-low border border-outline-variant/50 text-white rounded-xl px-4 py-3 text-sm focus:border-primary outline-none transition-all">
  </div>
  </div>
  ...
  <!-- Agente Asignado -->
  <div class="bg-gradient-to-br from-primary-container to-surface-container-highest border border-primary/20 rounded-2xl p-6 relative overflow-hidden">
  ...
  <button class="w-full py-2.5 bg-white text-primary-container hover:bg-gray-100 text-sm font-bold rounded-xl transition-colors flex items-center justify-center gap-2 relative z-10" <?php if(!$tiene_agente) echo 'disabled'; ?> onclick="Swal.fire({icon:'info', title:'Próximamente', text:'Chat directo en desarrollo.', background:'#071628', color:'#fff'})">
