...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['id'])) {
    ...
    $action = $_POST['action'];
    if ($action === 'aprobar') {
        $pdo->prepare("UPDATE solicitudes_registro SET estatus='APROBADA' WHERE id=?")->execute([$id]);
        $msgFlash = 'aprobada';
    } elseif ($action === 'rechazar') {
        $pdo->prepare("UPDATE solicitudes_registro SET estatus='RECHAZADA' WHERE id=?")->execute([$id]);
        $msgFlash = 'rechazada';
    }
...
// ── KPIs ──────────────────────────────────────────────────────────────────────
$pendientes = (int)$pdo->query("SELECT COUNT(*) FROM solicitudes_registro WHERE estatus='PENDIENTE'")->fetchColumn();
$aprobadas  = (int)$pdo->query("SELECT COUNT(*) FROM solicitudes_registro WHERE estatus='APROBADA'")->fetchColumn();
$rechazadas = (int)$pdo->query("SELECT COUNT(*) FROM solicitudes_registro WHERE estatus='RECHAZADA'")->fetchColumn();

// ── Lista ─────────────────────────────────────────────────────────────────────
$filtro = $_GET['filtro'] ?? '';
$where  = $filtro ? "WHERE estatus = " . $pdo->quote($filtro) : "";
$solicitudes = $pdo->query(
    "SELECT * FROM solicitudes_registro $where ORDER BY created_at DESC LIMIT 50"
)->fetchAll();
