...
$stmt = $pdo->prepare("SELECT p.*, c.razon_social, c.rfc, c.domicilio_fiscal, c.email, c.telefono_local FROM pedidos p JOIN clientes c ON p.cliente_id = c.id WHERE p.id = ?");
$stmt->execute([$id]);
$pedido = $stmt->fetch();
