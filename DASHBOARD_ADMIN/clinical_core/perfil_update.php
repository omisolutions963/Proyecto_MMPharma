...
if (!$adminId && !empty($_SESSION['admin_email'])) {
    $row = $pdo->prepare("SELECT id FROM admin_usuarios WHERE email=?");
...
if ($action === 'update_perfil') {
    ...
    $dup = $pdo->prepare("SELECT id FROM admin_usuarios WHERE email=? AND id<>?");
    $dup->execute([$email, $adminId]);
    if ($dup->fetchColumn()) {
        echo json_encode(['ok'=>false,'msg'=>'Ese email ya está en uso']); exit;
    }

    $pdo->prepare("UPDATE admin_usuarios SET nombre=?, email=? WHERE id=?")
...
if ($action === 'change_password') {
    ...
    $row = $pdo->prepare("SELECT password_hash FROM admin_usuarios WHERE id=?");
    $row->execute([$adminId]);
...
if ($action === 'upload_foto') {
    ...
    $oldRow = $pdo->prepare("SELECT foto_perfil FROM admin_usuarios WHERE id=?");
    ...
    $pdo->prepare("UPDATE admin_usuarios SET foto_perfil=? WHERE id=?")
